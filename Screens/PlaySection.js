...
import { View, Text, Alert, TouchableOpacity, StyleSheet, FlatList, Pressable, TextInput } from 'react-native';
...
const PlaySection = ({ navigation, route }) => {
    ...
    // console.log("called numbers length", gameData.calledNumbers.length);
    ...
    const [inputBoardNumberManually, setInputBoardNumberManually] = useState();
    ...
    const generateNumberOnManualTouch = (newNumber) => {
        if (dividendsNotification) return;
        if (calledNumbers.length >= 90) {
            const updatedGameData = {
                ...gameData,
                isGameCompleted: true,
        ...
        if (calledNumbers.includes(newNumber)) return;
        ...
        updatedPlayers.map((player) => {
            player?.tickets.map((ticket) => {
                ...
                if (
                    ...
                    const handleDividendSequence = () => {
                        const isKeyPresentInGroupedWinnersByDividendAndSequence = dividend.dividendName in gameData.groupedWinnersByDividendAndSequence;
                        if (isKeyPresentInGroupedWinnersByDividendAndSequence) {
                            const maxKey = Math.max(...Object.keys(gameData.groupedWinnersByDividendAndSequence[dividend.dividendName]).map(Number)).toString();
                    ...
                    const winnerData = {
                        ...
                        dividend: dividend,
                        dividendSequence: handleDividendSequence(),
                        isApproved: false,
                        ticket: ticket,
    ...
    const startGameOnBoardTouch = (number) => {
        if (!gameData.isRunning) {
            Alert.alert(
                "Are You Sure? You want to start the game?",
                "After that, You will not be able to add any player or do any change in the game.",
            ...
            generateNumberOnManualTouch(number);
        }
    }


    const handleEveryBoardNumberInputAlert = (number) => {
        Alert.alert(
            `You are entering ${number}`, // Title of the alert
    ...
    const handleInputBoardNumberManually = () => {
        if (!inputBoardNumberManually) {
            Alert.alert("Please enter any number from 1 to 90, both 1 and 90 are included.");
            setInputBoardNumberManually('');
            return;
        }
        const number = Number(inputBoardNumberManually);
        ...
        if (!gameData.isRunning) {
            ...
            handleEveryBoardNumberInputAlert(number);
        }
    ...
    const handleBoardTouchOnOff = () => {
        const updatedGameData = {
            ...gameData,
            isBoardTouchOn: !gameData.isBoardTouchOn
        };
        ...
        <View style={styles.container}>
            <View style={{ marginTop: 10, justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center' }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10, marginLeft: 15 }}>
                    <View>
                        <TextInput
                            ...
                            value={inputBoardNumberManually}
                            onChangeText={(text) => {
                                if (text.length <= 2) {
                                    setInputBoardNumberManually(text);
                                }
                            }}
                        />
                    </View>
                    <TouchableOpacity
                        onPress={handleInputBoardNumberManually}
                ...
                <TouchableOpacity
                    ...
                    <Text>{gameData.isBoardTouchOn ? <MaterialIcons name="touch-app" size={25} color="black" /> : <MaterialIcons name="do-not-touch" size={24} color="black" />}</Text>
                </TouchableOpacity>
                    ...
                    <View style={{ backgroundColor: '#f2f2f2' }}>
                        <View style={styles.numberBoard}>
                            {numberBoard.map(number => (
                                <Pressable
                                    onPress={() => {
                                        if (gameData.isBoardTouchOn) startGameOnBoardTouch(number)
                                        else return;
                                    }}
                                    key={number}
                                    style={[
