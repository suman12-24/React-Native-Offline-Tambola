const TambolaNumberSet = (randomTicketSize) => {
    // console.log('TambolaNumberSet called', randomTicketSize);
    ...
    const fillFunctions = [fillArray1, fillArray2, fillArray3, fillArray4];
    const randomFillFunction = fillFunctions[randomTicketSize];
...
// Helper function to generate the marked array (3x9, all false)
const generateMarkedArray = () => {
    return Array.from({ length: 3 }, () => Array(9).fill(false));
};

// Convert ticket array to a string hash for faster comparisons
const ticketToHash = (ticket) => {
    return ticket.flat().join(','); // Flatten the 2D array into a comma-separated string
};

const isTicketUnique = (newTicketHash, allTicketHashes) => {
    return !allTicketHashes.has(newTicketHash);
};

export const generateUniqueTickets = (numberOfTickets) => {
    if (numberOfTickets % 6 != 0) {
        var ticketQuantities = numberOfTickets + (6 - (numberOfTickets % 6));
    } else {
        var ticketQuantities = numberOfTickets;
    }
    const allTicketsArray = [];
    const allTicketHashes = new Set(); // Store ticket hashes for quick uniqueness checks
    let retries = 0;
    const maxRetries = 100; // Limit the number of retries for regeneration

    // Generate tickets in multiples of 6
    while (allTicketsArray.length < ticketQuantities && retries < maxRetries) {
        const randomTicketSize = Math.floor(Math.random() * 4);
        const newTicketSet = TambolaNumberSet(randomTicketSize); // Generate a new set of 6 tickets
        let allUnique = true;

        for (const ticket of newTicketSet) {
            const ticketHash = ticketToHash(ticket);
            if (!isTicketUnique(ticketHash, allTicketHashes)) {
                allUnique = false;
                break;
            }
        }
        ...
        if (allUnique) {
            // Add each ticket and its hash if all tickets are unique
            newTicketSet.forEach(ticket => {
                allTicketsArray.push({
                    ticket: ticket,           // The generated Bingo ticket (3x9 array)
                    marked: generateMarkedArray()  // A 3x9 array with all values set to `false`
                });
                allTicketHashes.add(ticketToHash(ticket));
            });
        } else {
            retries++; // Increment retry count if duplicates were found
        }

        if (retries >= maxRetries) {
            console.warn('Max retries reached while generating unique tickets');
            break;
        }
    }

    return allTicketsArray.slice(0, numberOfTickets);
};